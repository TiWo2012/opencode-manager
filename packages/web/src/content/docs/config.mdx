---
title: Config
description: Using the OpenCode Manager JSON config.
---

You can configure OpenCode Manager using a JSON config file.

---

## Format

OpenCode Manager supports both **JSON** and **JSONC** (JSON with Comments) formats.
...
4. **Project config** (`opencode-manager.json` in project) - project-specific settings
...
Place your global OpenCode Manager config in `~/.config/opencode/opencode.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.
...
Add `opencode-manager.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When OpenCode Manager starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Drop an `opencode-manager.json` or `opencode-manager.jsonc` file in the system managed config directory:
...
On macOS, OpenCode Manager reads managed preferences from the `ai.opencode.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

OpenCode Manager checks these paths:

1. `/Library/Managed Preferences/<user>/ai.opencode.managed.plist`
2. `/Library/Managed Preferences/ai.opencode.managed.plist`

The plist keys map directly to `opencode-manager.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.opencode.managed` PayloadType. The OpenCode Manager config keys go directly in the payload dict:
...
The server/runtime config schema is defined in [**`opencode-manager.ai/config.json`**](https://opencode-manager.ai/config.json).

TUI config uses [**`opencode-manager.ai/tui.json`**](https://opencode-manager.ai/tui.json).
...
```json title="tui.json"
{
  "$schema": "https://opencode-manager.ai/tui.json",
  "scroll_speed": 3,
  "scroll_acceleration": {
    "enabled": true
  },
...
Legacy `theme`, `keybinds`, and `tui` keys in `opencode-manager.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `opencode-manager serve` and `opencode web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your OpenCode Manager server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `opencode-manager.local`. Useful for running multiple instances on the same network.
...
If not specified, OpenCode Manager will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your OpenCode Manager config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, OpenCode Manager tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny OpenCode Manager actions on configured resources. Currently, policies can control which providers OpenCode Manager may use.
...
OpenCode Manager normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, OpenCode Manager omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, OpenCode Manager will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`opencode-manager run`), desktop app, and GitHub Action.
...
OpenCode Manager uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
OpenCode Manager will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
[Plugins](/docs/plugins) extend OpenCode Manager with custom tools, hooks, and integrations.
...
This is useful when you want to restrict OpenCode Manager to only use specific providers rather than disabling them one by one.
