...
Agent skills let OpenCode Manager discover reusable instructions from your repo or home directory.
...
OpenCode Manager searches these locations:
...
For project-local paths, OpenCode Manager walks up from your current working directory until it reaches the git worktree.
...
OpenCode Manager lists available skills in the `skill` tool description.
...
Control which skills agents can access using pattern-based permissions in `opencode-manager.json`:
...
**For built-in agents** (in `opencode-manager.json`):
