...
To debug issues with OpenCode Manager, start by checking the logs and local data it stores on disk.
...
To uninstall the OpenCode Manager CLI and remove its related files, run:
...
To remove OpenCode Manager Desktop, uninstall the application through your operating system's app management tools.

---

## Desktop app

OpenCode Manager Desktop runs a local OpenCode Manager server (the `opencode-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.

### Quick checks

- Fully quit and relaunch the app.
- If the app shows an error screen, click **Restart** and copy the error details.
- macOS only: `OpenCode Manager` menu -> **Reload Webview** (helps if the UI is blank/frozen).
...
OpenCode Manager can also load local plugins from disk. Temporarily move these out of the way (or rename the folder) and restart the desktop app:
...
If disabling plugins doesn't help (or a plugin install is stuck), clear the cache so OpenCode Manager can rebuild it.

1. Quit OpenCode Manager Desktop completely.
...
3. Restart OpenCode Manager Desktop.

---

### Fix server connection issues

OpenCode Manager Desktop can either start its own local server (default) or connect to a server URL you configured.
...
If your `opencode-manager.json(c)` contains a `server` section, temporarily remove it and restart the desktop app.
...
On Windows, OpenCode Manager Desktop requires the Microsoft Edge **WebView2 Runtime**. If the app opens to a blank window or won't start, install/update WebView2 and try again.

---

### Windows: General performance issues

If you're experiencing slow performance, file access issues, or terminal problems on Windows, try using [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL provides a Linux environment that works more seamlessly with OpenCode Manager's features.

---

### Notifications not showing

OpenCode Manager Desktop only shows system notifications when:

- notifications are enabled for OpenCode Manager in your OS settings, and
...
1. Quit OpenCode Manager Desktop.
2. Find and delete these files (they live in the OpenCode Manager Desktop app data directory):

- `opencode-manager.settings.dat` (desktop default server URL)
- `opencode-manager.global.dat` and `opencode-manager.workspace.*.dat` (UI state like recent servers/projects)
...
If you're experiencing issues with OpenCode Manager:
...
2. **Join our Discord**
   ...
   [**opencode-manager.ai/discord**](https://opencode-manager.ai/discord)
