---
title: CLI
description: OpenCode Manager CLI options and commands.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

The OpenCode Manager CLI by default starts the [TUI](/docs/tui) when run without any arguments.

```bash
opencode
```

But it also accepts commands as documented on this page. This allows you to interact with OpenCode Manager programmatically.
...
Start the OpenCode Manager terminal user interface.
...
The OpenCode Manager CLI also has the following commands.

---

### agent

Manage agents for OpenCode Manager.
...
Attach a terminal to an already running OpenCode Manager backend server started via `serve` or `web` commands.

```bash
opencode attach [url]
```

This allows using the TUI with a remote OpenCode Manager backend. For example:
...
OpenCode Manager is powered by the provider list at [Models.dev](https://models.dev), so you can use `opencode auth login` to configure API keys for any provider you'd like to use. This is stored in `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

When OpenCode Manager starts up it loads the providers from the credentials file. And if there are any keys defined in your environments or a `.env` file in your project.
...
Use the `--refresh` flag to update the cached model list. This is useful when new models have been added to a provider and you want to see them in OpenCode Manager.
...
You can also attach to a running `opencode-manager serve` instance to avoid MCP server cold boot times on every run:
...
Start a headless OpenCode Manager server for API access. Check out the [server docs](/docs/server) for the full HTTP interface.
...
Manage OpenCode Manager sessions.
...
List all OpenCode Manager sessions.
...
Delete an OpenCode Manager session.
...
Show token usage and cost statistics for your OpenCode Manager sessions.
...
Import session data from a JSON file or OpenCode Manager share URL.

```bash
opencode import <file>
```

You can import from a local file or an OpenCode Manager share URL.
...
Start a headless OpenCode Manager server with a web interface.

```bash
opencode web
```

This starts an HTTP server and opens a web browser to access OpenCode Manager through a web interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `opencode`).
...
Fetch and checkout a GitHub PR branch, then run OpenCode Manager.
...
Uninstall OpenCode Manager and remove all related files.
...
OpenCode Manager can be configured using environment variables.
