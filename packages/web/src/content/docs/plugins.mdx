---
title: Plugins
description: Write your own plugins to extend OpenCode Manager.
---

Plugins allow you to extend OpenCode Manager by hooking into various events and customizing behavior. You can create plugins to add new features, integrate with external services, or modify OpenCode Manager's default behavior.
...
2. Project config (`opencode-manager.json`)
...
OpenCode Manager runs `bun install` at startup to install these. Your plugins and tools can then import them.
...
:::note
If you’re using the OpenCode Manager desktop app, it can send system notifications automatically when a response is ready or when a session errors.
:::
...
Levels: `debug`, `info`, `warn`, `error`. See [SDK documentation](https://opencode-manager.ai/docs/sdk) for details.
