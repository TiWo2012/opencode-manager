---
title: Providers
description: Using any LLM provider in OpenCode Manager.
---

import config from "../../../config.mjs"
export const console = config.console

OpenCode Manager uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your OpenCode Manager config.
...
You can customize the providers through the `provider` section in your OpenCode Manager
config.
...
{
  "$schema": "https://opencode-manager.ai/config.json",
  "provider": {
    "anthropic": {
      "options": {
...
OpenCode Zen is a list of models provided by the OpenCode Manager team that have been
tested and verified to work well with OpenCode Manager. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with OpenCode Zen.
:::

1. Run the `/connect` command in the TUI, select `OpenCode Zen`, and head to [opencode-manager.ai/auth](https://opencode-manager.ai/zen).
...
It works like any other provider in OpenCode Manager and is completely optional to use.

---

## OpenCode Go

OpenCode Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the OpenCode Manager team that have been
tested and verified to work well with OpenCode Manager.

1. Run the `/connect` command in the TUI, select `OpenCode Go`, and head to [opencode-manager.ai/auth](https://opencode-manager.ai/zen).
...
To use Amazon Bedrock with OpenCode Manager:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `opencode-manager.json`:
...
OpenCode Manager. Anthropic explicitly prohibits this.

Previous versions of OpenCode Manager came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in OpenCode Manager with zero setup:
...
OpenCode Manager supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; OpenCode Manager uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize OpenCode Manager. Sign in and approve.

   :::note
   OpenCode Manager requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, OpenCode Manager surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — OpenCode Manager forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in OpenCode Manager is experimental. Features, configuration, and
behavior may change in future releases.
:::

OpenCode Manager integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
2. Choose your authentication method:
   ...
   3. Name: `OpenCode Manager`, Scopes: `api`
...
OpenCode Manager uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock OpenCode Manager
to only use your own GitLab-hosted instance, add the following to your
`opencode-manager.json` file. It is also recommended to disable session sharing.
...
Customize through `opencode-manager.json`:
...
When a `duo-workflow-*` model is selected, OpenCode Manager will:
...
4. Route tool execution requests through OpenCode Manager's permission-gated tool system
...
To use GMI Cloud with OpenCode Manager:
...
To use Google Vertex AI with OpenCode Manager:
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each OpenCode Manager conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each OpenCode Manager conversation listed as a separate session.
...
:::tip
Ollama can automatically configure itself for OpenCode Manager. See the [Ollama integration docs](https://docs.ollama.com/integrations/opencode) for details.
:::
...
To use Ollama Cloud with OpenCode Manager:
...
3. Copy the API key for use in OpenCode Manager.
...
6. **Important**: Before using cloud models in OpenCode Manager, you must pull the model information locally:
...
OpenCode Zen is a list of tested and verified models provided by the OpenCode Manager team. [Learn more](/docs/zen).
...
3. Enter your OpenCode Manager API key.
...
5. Complete sign-in in the browser that opens. OpenCode Manager captures the OAuth callback automatically and stores the token — no copy/paste needed.
...
The model catalog is provided automatically. A minimal `opencode-manager.json` is all that's needed:
...
2. Select **xAI Grok OAuth (SuperGrok Subscription)**. OpenCode Manager opens xAI's consent screen in your browser and waits for the callback on `http://127.0.0.1:56121/callback`.
...
OpenCode Manager refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.

#### Option B — SuperGrok device-code (headless / remote server / VPS)

Use this when OpenCode Manager is running somewhere a browser can't reach the loopback redirect: a VPS, a remote dev box over SSH, inside Docker, in CI, etc. No callback port is opened on the host running OpenCode Manager — instead xAI hands the CLI a short code that you type into a browser on any other device (laptop, phone, …).
...
2. Select **xAI Grok OAuth (Headless / Remote / VPS)**. OpenCode Manager prints a verification URL and a short user code.

   ```txt
   Open https://x.ai/device on any device and enter code: ABCD-1234
   ```

3. Open the URL on a device that has a browser (your laptop or phone), enter the code, and approve the consent screen. OpenCode Manager polls xAI's token endpoint and stores the resulting OAuth tokens once you approve. Token refresh works the same as Option A.
...
4. Create or update your `opencode-manager.json` file in your project directory:
...
The `limit` fields allow OpenCode Manager to understand how much context you have left. Standard providers pull these from models.dev automatically.
