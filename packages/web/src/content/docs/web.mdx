---
title: Web
description: Using OpenCode Manager in your browser.
---

OpenCode Manager can run as a web application in your browser, providing the same powerful AI coding experience without needing a terminal.

![OpenCode Manager Web - New Session](../../assets/web/web-homepage-new-session.png)
...
This starts a local server on `127.0.0.1` with a random available port and automatically opens OpenCode Manager in your default browser.
...
By default, OpenCode Manager picks an available port. You can specify a port:
...
By default, the server binds to `127.0.0.1` (localhost only). To make OpenCode Manager accessible on your network:

```bash
opencode web --hostname 0.0.0.0
```

When using `0.0.0.0`, OpenCode Manager will display both local and network addresses:
...
This automatically sets the hostname to `0.0.0.0` and advertises the server as `opencode-manager.local`.
...
Once started, the web interface provides access to your OpenCode Manager sessions.

### Sessions

View and manage your sessions from the homepage. You can see active sessions and start new ones.

![OpenCode Manager Web - Active Session](../../assets/web/web-homepage-active-session.png)

### Server Status

Click "See Servers" to view connected servers and their status.

![OpenCode Manager Web - See Servers](../../assets/web/web-homepage-see-servers.png)
...
You can also configure server settings in your `opencode-manager.json` config file:
