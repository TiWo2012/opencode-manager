...
The `opencode-manager serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an opencode client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `opencode-manager.local` |
...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `opencode`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `opencode-manager serve` and `opencode web`.
...
You can run `opencode-manager serve` to start a standalone server. If you have the
opencode TUI running, `opencode-manager serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the OpenCode Manager [IDE](/docs/ide) plugins.
