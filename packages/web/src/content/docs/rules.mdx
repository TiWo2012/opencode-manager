...
For users migrating from Claude Code, OpenCode Manager supports Claude Code's file conventions as fallbacks:
...
You can specify custom instruction files in your `opencode-manager.json` or the global `~/.config/opencode/opencode.json`. This allows you and your team to reuse existing rules rather than having to duplicate them to AGENTS.md.
...
The recommended approach is to use the `instructions` field in `opencode-manager.json`:
...
:::tip
For monorepos or projects with shared standards, using `opencode-manager.json` with glob patterns (like `packages/*/AGENTS.md`) is more maintainable than manual instructions.
:::
