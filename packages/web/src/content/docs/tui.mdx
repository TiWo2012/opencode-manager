---
title: TUI
description: Using the OpenCode Manager terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenCode Manager provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running OpenCode Manager starts the TUI for the current directory.
...
When using the OpenCode Manager TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to OpenCode Manager. Allows you to select from available providers and add their API keys.
...
Exit OpenCode Manager. _Aliases_: `/quit`, `/q`
...
This is separate from `opencode-manager.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long OpenCode Manager waits after the leader key. Defaults to `2000`.
...
- `sound_pack` - Sound pack ID to use. Defaults to `opencode-manager.default`.
