...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `opencode-manager auth login` in the terminal",
      name: "Login with opencode-manager",
      id: AuthMethodID,
    }
    ...
    if (params.clientCapabilities?._meta?.["terminal-auth"] === true) {
      authMethod._meta = {
        "terminal-auth": {
          command: "opencode-manager",
          args: ["auth", "login"],
          label: "OpenCode Manager Login",
        },
      }
    }
...
async function loadDirectorySnapshot(sdk: OpencodeClient, directory: string) {
  return ACPProfile.measure("acp.directory.load", async () => {
    ...
    return Directory.build({
      ...
      providers,
      modes,
      defaultModeID: agents.find((agent) => agent.mode === "primary" && agent.hidden !== true)?.name ?? "orchestrator",
...
function promptErrorMessage(error: AssistantError) {
  ...
  return "OpenCode Manager prompt failed"
}
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "OpenCode Manager service failure", service })
}
